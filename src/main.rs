use std::{io::{Write, stdout}, path::Path, process::Command, fs::{self, File}, thread::{self, sleep}, sync::Arc, collections::BTreeMap, time::Duration};
                ...
                let mut frame: Vec<(u32,u32,String)> = new_pixels.into_values().map(|pixel|{
                    ...
                    for i in 0..2 {
                        ...
                        (&mapping).iter().for_each(|txt|{
                            let mut offset = t.abs_diff(txt.t as u16);
                            offset += b.abs_diff(txt.b as u16);
